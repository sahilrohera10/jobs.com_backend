...
async function sendOtpforgetPassword(req, resp) {
  ...
  try {
    ...
    if (data) {
      ...
      console.log(text);
      sendMail(body.user_email,sub,text);
      await user_model.findOneAndUpdate({user_email:body.user_email},{user_otp:otp});
  ...
  try {
    ...
    if (data) {
      if(body.user_otp==data.user_otp){
        await user_model.findOneAndUpdate({user_email:body.user_email},{user_password:body.user_password});
